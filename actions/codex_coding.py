"""Sandboxed Codex coding action used by Anubis."""
...
READ_ONLY_ACTIONS = {"explain", "review", "analyze"}
...
def _build_prompt(parameters: dict, root: Path) -> str:
    ...
    description = str(
        parameters.get("description") or parameters.get("instruction") or ""
    ).strip()
    lines = [f"Coding action: {action}", f"Request: {description or 'Complete the requested coding task.'}"]
    ...
    lines.append(
        ...
        "For explain/review actions, inspect and report without changing files."
    )
    return "\n\n".join(lines)
...
def codex_coding(
    ...
    try:
        ...
        return "Codex SDK is not installed. Run: python -m pip install openai-codex"

    try:
        root = _workspace_root()
        workdir = _working_directory(parameters, root)
        prompt = _build_prompt(parameters, root)
        sandbox = Sandbox.read_only if action in READ_ONLY_ACTIONS else Sandbox.workspace_write

        if speak:
            speak("Starting the sandboxed coding task, sir.")

        with Codex(CodexConfig(cwd=str(workdir))) as codex:
