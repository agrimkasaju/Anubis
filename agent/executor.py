...
def _run_codex_fallback(description: str, speak: Callable | None = None) -> str:
    from actions.codex_coding import codex_coding

    return codex_coding(
        parameters={"action": "auto", "description": description},
        speak=speak,
    )
...
def _detect_language(text: str) -> str:
    model = GeminiModel(get_gemini_lite_model())
    try:
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool == "open_app":
    ...
    elif tool == "codex_coding":
        from actions.codex_coding import codex_coding
        return codex_coding(parameters=parameters, player=None, speak=speak) or "Done."
        ...
        return _run_codex_fallback(description, speak=speak)

    elif tool == "flight_finder":
        from actions.flight_finder import flight_finder
        return flight_finder(parameters=parameters, player=None, speak=speak) or "Done."

    elif tool == "get_stock_analysis":
        from actions.stock_actions import get_stock_analysis
        return get_stock_analysis(symbol=parameters.get("symbol", "")) or "Done."

    else:
        print(f"[Executor] ⚠️ Unknown tool '{tool}' — falling back to Codex")
        return _run_codex_fallback(f"Accomplish this task: {parameters}", speak=speak)
